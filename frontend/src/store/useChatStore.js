...
export const useChatStore = create((set, get) => ({
  ...
  deleteMessages: async (messageIds) => {
    try {
      ...
      set({ messages: messages.filter((msg) => !messageIds.includes(msg._id)) });
  ...
  subscribeToMessages: () => {
    ...
    socket.on("userUpdated", (updatedUser) => {
      ...
      const isContact = allContacts.some(c => c._id === updatedUser._id);
      if (isContact) {
        set({ allContacts: allContacts.map(c => c._id === updatedUser._id ? updatedUser : c) });
      }

      // Update in chats
      const isChatPartner = chats.some(c => c._id === updatedUser._id);
      if (isChatPartner) {
        set({ chats: chats.map(c => c._id === updatedUser._id ? updatedUser : c) });
      }
